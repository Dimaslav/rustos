...
pub extern "C" fn syscall_dispatch(regs: *mut u64) {
    ...
    let result: u64 = match nr {
        ...
        SYS_SPAWN => match crate::user::spawn_worker_elf() {
            Ok(id) => id,
            Err(e) => {
                crate::log_warn!("[syscall] spawn worker failed: {:?}", e);
                u64::MAX
            }
        },
        ...
        SYS_SET_FOCUS => {
            crate::keyboard::set_user_owns(a1 != 0);
            0
        }
        SYS_SPAWN_CALC => match crate::user::spawn_calculator_elf() {
            Ok(id) => id,
            Err(e) => {
                crate::log_warn!("[syscall] spawn calc failed: {:?}", e);
                u64::MAX
            }
        },
        SYS_CREATE_WINDOW => sys_create_window(a1, a2, a3),
        SYS_DESTROY_WINDOW => {
            if crate::win::destroy(a1) { 0 } else { u64::MAX }
        }
        SYS_PRESENT => {
            if crate::win::present(a1) { 0 } else { u64::MAX }
        }
        SYS_POLL_EVENT => sys_poll_event(a1, a2),
        _ => {
            crate::log_warn!("[syscall] unknown nr={}", nr);
            u64::MAX
        }
    };

    unsafe {
        *regs.add(REG_RAX) = result;
    }
}
...
fn sys_open(path_ptr: u64, path_len: u64) -> u64 {
    ...
    let data = match crate::vfs::read(path) {
        Ok(d) => d,
        Err(_) => {
            crate::log_warn!("[syscall] open({}) — not found", path);
            return u64::MAX;
        }
    };
    ...
    for (i, slot) in table.iter_mut().enumerate() {
        if slot.is_none() {
            *slot = Some(FileHandle { name, data, pos: 0 });
            return i as u64;
        }
    }
    table.push(Some(FileHandle { name, data, pos: 0 }));
    (table.len() - 1) as u64
}
...
fn sys_exec(name_ptr: u64, name_len: u64) -> u64 {
    ...
    match crate::user::exec_by_name(name) {
        Ok(id) => id,
        Err(e) => {
            crate::log_warn!("[syscall] exec({}) failed: {:?}", name, e);
            u64::MAX
        }
    }
}
